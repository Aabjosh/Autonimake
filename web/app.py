...
@app.route('/api/model_exists', methods=['GET'])
def model_exists():
...
def train_trigger():
    ...
    try:
        process = subprocess.Popen(
            [sys.executable, script_path, label],
            cwd=BACKEND_DIR,
...
def reset_dataset():
    try:
        for d in [HAND_DATASET_DIR, OBJ_DATASET_DIR]:
            if os.path.exists(d):
                shutil.rmtree(d)
...
def build_model():
    ...
    # Skip training if model already exists and force is False
    if not force and os.path.exists(MODEL_PATH):
        return jsonify({"message": "Model already exists. Skipping training.", "skipped": True}), 200
    ...
    try:
        ...
        if process.returncode == 0:
            return jsonify({"message": "Model trained successfully!", "skipped": False}), 200
        else:
            ...
            if "Need at least 2 populated classes" in err_text:
                return jsonify({
                    "error": "Need at least 2 identity tags with images.",
